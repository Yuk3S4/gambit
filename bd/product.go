...
func SelectProduct(p models.Product, choice string, page, pageSize int, orderType, orderField string) (models.ProductResp, error) {
	...
	case "S":
		where = "WHERE UCASE(CONCAT(Prod_Id, Prod_Description)) LIKE '%" + strings.ToUpper(p.ProdSearch) + "%' "
	case "C":
	...
	for rows.Next() {
		...
		p.ProdUpdatedAt = ProdUpdated.Time.String()
	...
	fmt.Println("SelectProduct > Ejecución Exitosa")
	return Resp, nil
}

func UpdateStock(p models.Product) error {
	fmt.Println("Comienza UpdateStock")

	if p.ProdStock == 0 {
		return errors.New("[ERROR] Debe de enviar el Stock a modificar")
	}
	...
	fmt.Println("UpdateStock > Ejecución Exitosa")
	return nil
}
